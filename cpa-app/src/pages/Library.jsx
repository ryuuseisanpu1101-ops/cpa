              ...
              {qs.map(q => {
                ...
                return (
                  <div key={q.id} style={{
                    ...
                    borderRadius: 10, padding: '9px 12px', marginBottom: 4,
                  }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: 9, color: '#1f2937', marginTop: 2 }}>{q.source === 'pdf' ? '📄' : '✎'} {atts.length}回解答{rate !== null ? ` · ${rate}%` : ''}</div>
                    </div>
...
function PdfExtract({ data, update }) {
  ...
  const [useLimbs, setUseLimbs] = useState(false)
  const [loading,  setLoading]  = useState(false)
  const [progress, setProgress] = useState(0)
  const [preview,  setPreview]  = useState(null)
  const [error,    setError]    = useState('')
  const [mode,     setMode]     = useState('pdf') // pdf | text | screenshot
  const [rawText,  setRawText]  = useState('')
  const [isGooднotes, setIsGoodnotes] = useState(false)
  const fileRef = useRef()

  const systemPrompt = isGoodnotes
    ? `あなたは公認会計士試験問題集の読み取りアシスタントです。
GoodNotesで手書き書き込みされたPDFから、問題の情報と書き込まれた正誤記号を読み取ってください。
必ず以下のJSON配列のみを返してください。他の文字は一切出力しないでください。
...
[
  {
    ...
    "importance": "B",
    "body": "問題文（あれば）",
    "pastResult": "correct|partial|wrong|null"
  }
]

- pastResult: 手書きの○→correct、△→partial、✕→wrong、書き込みなし→null
- 日付は無視
- 問題文が読み取れない場合はbodyを空文字に
- JSONのみ出力`
    : `あなたは公認会計士試験問題集の読み取りアシスタントです。
PDFから問題を抽出し、必ず以下のJSON配列のみを返してください。他の文字は一切出力しないでください。

[
  {
    "number": "1",
    "limbIndex": null,
    "topicName": "${topic || '指定された論点'}",
    "importance": "B",
    "body": "問題文（肢を含む完全な問題文）"
  }
]
...
- numberは問題番号（文字列）
- limbIndexは肢番号（1問複数肢の場合）、なければnull
- importanceはA/B/C（記載がなければBと推定）
- bodyは問題文全体（選択肢含む）
- JSONのみ出力、コードブロック不要`
    ...
    if (isGoodnotes) {
      preview.forEach((p, i) => {
        if (p.pastResult && p.pastResult !== 'null') {
          newAttempts.push({
            id: uid(), questionId: newQs[i].id,
            date: today, result: p.pastResult, memo: 'GoodNotes読み取り', rationale: '',
          })
        }
      })
    }
  ...
  return (
    <div>
      <Card style={{ marginBottom: 12 }}>
        ...
        {/* GoodNotes toggle */}
        <div onClick={() => setIsGoodnotes(v => !v)} style={{
          display: 'flex', alignItems: 'center', gap: 10, padding: '8px 12px',
          background: isGoodnotes ? '#c084fc18' : 'rgba(255,255,255,0.04)',
          border: `1px solid ${isGoodnotes ? '#c084fc40' : C.border}`,
          borderRadius: 10, cursor: 'pointer', marginBottom: 10,
        }}>
          <div style={{
            width: 20, height: 20, borderRadius: 6,
            background: isGoodnotes ? '#c084fc' : 'rgba(255,255,255,0.1)',
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
          {[{ key: 'pdf', label: '📄 PDF' }, { key: 'text', label: '📝 テキスト' }].map(m => (
            <button key={m.key} onClick={() => setMode(m.key)} style={{
              ...
              border: `1px solid ${mode === m.key ? '#38bdf8' : C.border}`,
              background: mode === m.key ? '#38bdf818' : 'rgba(255,255,255,0.03)',
              color: mode === m.key ? '#38bdf8' : C.muted,
            }}>{m.label}</button>
          ))}
        </div>
        ...
        {mode === 'pdf' ? (
          <div>
            ...
            <div onClick={() => fileRef.current.click()} style={{
              ...
              <div style={{ fontSize: 10, color: '#1f2937', marginTop: 3 }}>章単位で送ると精度が上がります</div>
            </div>
          </div>
        ) : (
          <div>
            ...
            <Btn onClick={extractFromText} full color="#38bdf8" style={{ marginTop: 8 }} disabled={loading || !rawText.trim()}>
              ✦ Claudeで抽出
            </Btn>
          </div>
        )}
          ...
          {preview.slice(0, 5).map((p, i) => (
            <Card key={i} style={{ marginBottom: 6, padding: '10px 12px' }}>
              <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                <span style={{ fontSize: 12, fontWeight: 800, color: '#e2e8f0' }}>問{p.number}{p.limbIndex ? `-${p.limbIndex}` : ''}</span>
  ...
  const add = (cont = false) => {
    ...
    update(prev => ({
      ...
      questions: [...prev.questions, {
        id: uid(), subject: sub, chapter: chapter.trim(),
        topicName: topic.trim(), number: num.trim(),
        limbIndex: limb.trim() || null,
        body: body.trim(), importance: imp,
        source: 'manual', createdAt: todayStr(),
      }],
    }))
    if (cont) { setNum(''); setBody(''); setLimb('') }
    else { setNum(''); setBody(''); setLimb(''); setTopic('') }
  }
  ...
  return (
    <Card>
          ...
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 5 }}>
            {existTopics.map(t => (
              <button key={t} onClick={() => setTopic(t)} style={{
                ...
                border: '1px solid rgba(255,255,255,0.08)', background: topic === t ? '#c084fc18' : 'rgba(255,255,255,0.04)',
                color: topic === t ? '#c084fc' : C.muted, fontFamily: 'inherit', cursor: 'pointer',
              }}>{t}</button>
            ))}
          </div>
        )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginTop: 10 }}>
        <div>
          <div style={{ fontSize: 10, color: C.muted, marginBottom: 4 }}>問題番号 *</div>
          <input value={num} onChange={e => setNum(e.target.value)} placeholder="1" style={iStyle} />
        </div>
        <div>
          <div style={{ fontSize: 10, color: C.muted, marginBottom: 4 }}>肢番号</div>
          <input value={limb} onChange={e => setLimb(e.target.value)} placeholder="1" style={iStyle} />
        </div>
        <div>
...
function Flashcards({ data, update }) {
  ...
  const add = () => {
    ...
    update(prev => ({
      ...prev,
      flashcards: [...(prev.flashcards || []), { id: uid(), subject: sub, front: front.trim(), back: back.trim(), category: cat.trim() }],
    }))
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 5, overflowX: 'auto', scrollbarWidth: 'none', marginBottom: 10 }}>
        {['', ...SUBJECTS].map((s, i) => (
          <FilterPill key={i} label={s || '全科目'} active={sub === s} color={s ? SUB[s]?.accent : '#38bdf8'} onClick={() => setSub(s)} />
        ))}
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        {filtered.map(card => {
          ...
          return (
            <div key={card.id} onClick={() => setFlip(f => ({ ...f, [card.id]: !f[card.id] }))} style={{
              background: isFlipped ? `${c.dim}` : C.card,
              border: `1px solid ${isFlipped ? c.accent + '40' : C.border}`,
              ...
              <div style={{ fontSize: 12, fontWeight: 700, color: isFlipped ? c.accent : '#e2e8f0', lineHeight: 1.5 }}>
...
function LawTracker({ data, update }) {
  ...
  const add = () => {
    ...
    update(prev => ({
      ...prev,
      articles: [...(prev.articles || []), { id: uid(), subject: '企業法', number: num.trim(), title: title.trim(), memo: memo.trim(), mastered: false }],
    }))
  ...
  const toggle = (id) => {
    update(prev => ({
      ...prev,
      articles: (prev.articles || []).map(a => a.id === id ? { ...a, mastered: !a.mastered } : a),
    }))
  }

  const iStyle = { padding: '9px 12px', borderRadius: 8, border: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.05)', color: '#e2e8f0', fontFamily: 'inherit', fontSize: 13, outline: 'none', width: '100%', boxSizing: 'border-box' }

  const mastered = articles.filter(a => a.mastered).length
    ...
    <div>
      <Card style={{ marginBottom: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <div style={{ fontSize: 12, color: '#4ade80', fontWeight: 700 }}>{mastered}/{articles.length} 習熟</div>
        </div>
      ...
      {filtered.map(a => (
        <div key={a.id} onClick={() => toggle(a.id)} style={{
          ...
          padding: '10px 14px', background: a.mastered ? '#14532d22' : C.card,
          border: `1px solid ${a.mastered ? '#22c55e30' : C.border}`,
          borderRadius: 12, marginBottom: 6, cursor: 'pointer',
        }}>
          <div style={{ width: 20, height: 20, borderRadius: 6, background: a.mastered ? '#22c55e' : 'rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#fff', flexShrink: 0 }}>
            {a.mastered ? '✓' : ''}
          </div>
          <div style={{ flex: 1 }}>
